        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {displayProducts.map((p, i) => (
              <ProductCard key={p.name} p={p} delay={i * 0.08} />
            ))}
          </div>
          <motion.div className="text-center mt-12" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
            <Link
              to="/productos"
              className="inline-flex items-center gap-2 border border-foreground text-foreground text-xs tracking-wider uppercase px-7 py-3.5 rounded-sm hover:bg-foreground hover:text-background transition-all cursor-pointer"
            >
              Ver más
